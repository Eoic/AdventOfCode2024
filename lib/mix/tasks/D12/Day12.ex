defmodule Mix.Tasks.Day12 do
  ...
  def reduce_edges(edges) do
  end

  def create_edges(positions) do
    IO.inspect(positions, label: "Provided positions")
    ...
    |> Kernel.then(fn edges ->
      # IO.inspect(length(edges), label: "OK")
      # TODO: Remove all points with frequency >= 2.
      # Flatten entire points list and sort by [x, y].
      # Take unique points.
      # Trace path from sta
      IO.inspect(Enum.frequencies(edges))
      edges
    end)
  end
    ...
    |> Enum.reduce(0, fn {label, count}, price ->
      ...
      prices =
        ...
        |> Enum.map(fn cluster ->
          ...
          IO.inspect(create_edges(border_points))
          # IO.inspect([label, border_points], label: "Border")
